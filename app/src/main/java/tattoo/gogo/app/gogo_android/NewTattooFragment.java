...
import tattoo.gogo.app.gogo_android.api.TattooApi;
...
public class NewTattooFragment extends NewWorkFragment {
    ...
    private void sendToApi() {
        TattooApi.getApi().tattoo(ThreadLocalRandom.current().nextInt(0, 10000), mTattoo)
                .enqueue(new Callback<List<Tattoo>>() {
            @Override
            public void onResponse(Call<List<Tattoo>> call, Response<List<Tattoo>> response) {
                List<Tattoo> tattoos = response.body();
                Snackbar.make(etAuthor, tattoos.get(tattoos.size()-1).getTitle(), Snackbar.LENGTH_LONG).show();
            }
